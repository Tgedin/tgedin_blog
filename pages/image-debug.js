...
export default function ImageDebug() {
  ...
  const testImage = (name, src) => {
    const img = new Image();
    img.onload = () => setImageStatuses(prev => ({ ...prev, [name]: "Loaded" }));
    img.onerror = () => setImageStatuses(prev => ({ ...prev, [name]: "Failed" }));
    img.src = src;
  };
  ...
  return (
    <MainLayout title="Image Debug">
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "2rem" }}>
        <thead>
          <tr>
            <th style={{ textAlign: "left", padding: "0.5rem", borderBottom: "1px solid #ccc" }}>Image</th>
        ...
        <tbody>
          {imagesToTest.map(img => (
            <tr key={img.name}>
              <td style={{ padding: "0.5rem", borderBottom: "1px solid #ccc" }}>{img.name}</td>
              <td style={{ padding: "0.5rem", borderBottom: "1px solid #ccc" }}>{img.src}</td>
      ...
      <ul>
        <li>If WebP images fail but JPG/JPEG succeed: Your browser might not support WebP</li>
        <li>If all images fail: Check the public directory path</li>
        <li>If some work and others don't: Check if those specific files exist</li>
      </ul>
    </MainLayout>
  );
}
